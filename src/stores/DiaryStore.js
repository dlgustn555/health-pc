...
export const createDiaryStore = () => ({
    ...
    async getMonthDiaries() {
        ...
        const {success, result:{data}} = await api.get(`/diary/month/diaries`, { params: {year, month}})
    ...
    async addProgramName({year, month, date, order, program}) {
        const {success, result: {data}} = await api.post('/diary/program/name', {year, month, date, order, program})
        if (!success) {
            return
        }
    ...
    async updateProgramName({_id, order, program}) {
        const {success, result: {data}} = await api.patch('/diary/program/name', {_id, order, program})
        if (!success) {
            return
        }
    ...
    // 선택한 다이어리를 조회한다.
    async getDiary({_id}) {
        const {success, result: {data}} = await api.get(`/diary/${_id}`)
    ...
    async updateProgram({order, program, type}) {
        ...
        const {success, result: {data}} = await api.patch('/diary/program/content', {_id, order, program, type})
        
        const isNewProgramAdd = this.diary[type].length === order

        if (success) {
            const updatedProgram = (isNewProgramAdd) ? [...this.diary[type], data] : this.diary[type].map((p, index) => ( (index === order) ? data : p))
            this.diary = {
                ...this.diary,
                [type]: [...updatedProgram]
            }
